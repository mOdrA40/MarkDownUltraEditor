...
export const useFileStorage = (): UseFileStorageReturn => {
  ...
  const [_isClerkStable, setIsClerkStable] = useState(false);
    ...
    const initializeStorageService = async () => {
      try {
        ...
        if (isSignedIn && userId) {
          safeConsole.log('🔐 User is signed in, attempting cloud storage setup');
          let retryCount = 0;
          const maxRetries = 3;

          while (retryCount < maxRetries) {
            try {
              safeConsole.log(`🔄 Authentication attempt ${retryCount + 1}/${maxRetries}`);

              const token = await getToken();
              safeConsole.log(
                '🎫 Token status:',
                token ? `✓ Available (${token.substring(0, 20)}...)` : '✗ Not available'
              );
              ...
              safeConsole.error(`❌ Authentication attempt ${retryCount} failed:`, {
                error: tokenError,
                message: errorMessage,
                code:
                  tokenError instanceof Error && 'code' in tokenError
                    ? (tokenError as Error & { code?: string }).code
                    : undefined,
              });

              if (retryCount >= maxRetries) {
                safeConsole.error('💥 Max retries exceeded, using local storage as fallback');
                supabaseClient = null;
                currentUserId = null;
        ...
        safeConsole.log('📦 Storage service created:', {
          ...
          serviceType: service && supabaseClient && currentUserId ? 'cloud' : 'local',
        });
  ...
  }, [isSignedIn, userId, getToken, queryClient.invalidateQueries]);
  ...
  const { data: storageInfo = null, isLoading: isLoadingStorageInfo } = useQuery({
    ...
    queryFn: async () => {
      ...
      safeConsole.log('📊 Service auth state:', {
        isAuthenticated:
          'isAuthenticated' in storageService
            ? (storageService as { isAuthenticated: boolean }).isAuthenticated
            : false,
        hasSupabaseClient:
          'supabaseClient' in storageService
            ? !!(storageService as { supabaseClient: unknown }).supabaseClient
            : false,
        hasUserId:
          'userId' in storageService ? !!(storageService as { userId: unknown }).userId : false,
      });
      ...
      try {
        ...
        try {
          ...
          safeConsole.error('💥 Fallback storage info also failed:', fallbackError);
          return null;
        }
      }
    },
  ...
  }, [isSignedIn, isInitialized, isAuthenticationComplete, isLoadingStorageInfo, storageInfo]);
