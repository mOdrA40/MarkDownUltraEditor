...
export const useFileActions = ({
  ...
  const handleOpenFile = useCallback(
    async (file: FileData) => {
      await setActiveFileFromFilesPage(file.id || file.title, file.title);

      const targetFile = file.id || file.title;
      const targetTitle = file.title;
      navigate(`/?file=${targetFile}&title=${targetTitle}`);
    },
    [setActiveFileFromFilesPage, navigate]
  );
