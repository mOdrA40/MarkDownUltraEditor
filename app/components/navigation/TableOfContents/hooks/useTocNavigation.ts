...
import { scrollToHeadingGlobal , HeadingItem } from '@/utils/headingUtils';
import { announceToScreenReader } from '@/utils/accessibility';
import { useScrollSpy, useKeyboardNavigation } from '@/hooks/navigation';
import { TocNavigationOptions, TocScrollSpyOptions, TocKeyboardOptions } from '../types/toc.types';
...
export const useTocNavigation = (
   ...
   * @param options - Navigation options
   */
  const handleHeadingClick = useCallback(async (
    headingId: string,
   ...
   * @param options - Navigation options
   */
  const navigateToNext = useCallback((options: TocNavigationOptions = {}) => {
   ...
   * @param options - Navigation options
   */
  const navigateToPrevious = useCallback((options: TocNavigationOptions = {}) => {
    const currentIndex = tocItems.findIndex(item => item.id === activeId);
   ...
   * @param options - Navigation options
   */
  const navigateToFirst = useCallback((options: TocNavigationOptions = {}) => {
   ...
   * @param options - Navigation options
   */
  const navigateToLast = useCallback((options: TocNavigationOptions = {}) => {
    if (tocItems.length > 0) {
