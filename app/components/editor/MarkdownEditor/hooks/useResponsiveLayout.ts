...
export const useResponsiveLayout = (): UseResponsiveLayoutReturn => {
  ...
  const [responsiveState, setResponsiveState] = useState<ResponsiveState>({
    ...
    isSmallTablet:
      initialDimensions.width >= 500 && initialDimensions.width <= 768,
  ...
  const getLayoutConfig = useCallback(
    (deviceType: DeviceType): LayoutConfig => {
      switch (deviceType) {
   ...
   */
  const getComponentSizing = useCallback(
    (deviceType: DeviceType): ComponentSizing => {
      switch (deviceType) {
        case DeviceType.MOBILE:
          return {
            buttonSize: "sm",
  ...
  const utils = {
    ...
    isAtLeast: (breakpoint: keyof typeof BREAKPOINTS) =>
      screenWidth >= BREAKPOINTS[breakpoint],
